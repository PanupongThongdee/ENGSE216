...
public class list {
   ...
   public void insert(int k, Record profile){
        ...
        for(int i = count ; i > k  ; i--){
            ...
            this.profile[i+1] = this.profile[i];
   ...
   void bubble_sort() {
    for (int i = 0; i < count; i++) {
        for (int j = 0; j < count - i - 1; j++) {
            ...
            String currentName = (this.profile[j].getFirstname() + " " + this.profile[j].getLastname()).toUpperCase();
            String nextName = (this.profile[j + 1].getFirstname() + " " + this.profile[j + 1].getLastname()).toUpperCase();
         ...
         void showall() {
           ...
           for (int i = 0; i < count; i++) {
            bubble_sort();
            System.out.printf("  %-5s %-15s %-15s %-15s\n", i + 1, this.profile[i].getFirstname(),this.profile[i].getLastname(),this.profile[i].getTel());
